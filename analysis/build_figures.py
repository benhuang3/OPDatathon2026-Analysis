...
# Color system — magenta / dark-pink dark theme
PALETTE = {
    "primary": "#FF4D9D",        # hot pink (high-emphasis)
    "accent": "#E91E63",         # magenta
    "secondary": "#4DD0E1",      # bright teal (complement)
    "muted": "#A78BA0",          # dusty mauve
    "skeptic": "#7E57C2",        # deep violet
    "late": "#F5B041",           # warm gold
    "consumer": "#EC4899",       # bright magenta
    "gn": "#9575CD",             # lavender
    "gs": "#FF7AC6",             # light pink
    "background": "#1A1024",     # deep aubergine
    "text": "#F0E5EC",           # warm white
    "muted_text": "#B5A0AB",     # muted mauve
    "axis": "#7A5A6E",
    "grid": "#2C1F33",
}
...
def fig1_aspiration_inversion(findings):
    ...
    width = 0.35
    axes[0].barh(y - width/2, use, width, color=PALETTE["muted"], label="Use today", edgecolor="#1A1024")
    axes[0].barh(y + width/2, want, width, color=PALETTE["accent"], label="Want", edgecolor="#1A1024")
    ...
    axes[1].errorbar(gap, y, xerr=[err_lo, err_hi], fmt='o',
                       color="#F0E5EC", ecolor="#B5A0AB", capsize=3,
                       markersize=8, markerfacecolor="white", markeredgewidth=2)
    for yi, gi, col in zip(y, gap, colors):
        axes[1].plot(gi, yi, 'o', color=col, markersize=8, zorder=3)
    axes[1].axvline(0, color="#6E5060", linewidth=0.7)
    axes[1].axvline(0.30, color="#3A2A36", linewidth=0.5, linestyle="--")
...
def fig3_lca_profile(df):
    ...
    ax.axvline(len(u_cols) - 0.5, color="#6E5060", linestyle="--", linewidth=0.7)
    ax.text(len(u_cols)/2 - 0.5, 1.08, "Currently uses AI for…",
            ha="center", fontsize=9, color="#E0D0DA", transform=ax.get_xaxis_transform())
    ax.text(len(u_cols) + len(w_cols)/2 - 0.5, 1.08, "Wants to use AI for…",
            ha="center", fontsize=9, color="#E0D0DA", transform=ax.get_xaxis_transform())
...
def fig4_oaxaca_smallorg(findings):
    ...
    bars = ax.bar(components, values, color=colors, edgecolor="#1A1024")
    ax.axhline(0, color="#F0E5EC", linewidth=0.6)
...
def fig5_irt_readiness(findings):
    ...
    sc = ax.scatter(b, a, s=80, color=PALETTE["accent"], edgecolor="#F0E5EC", linewidth=0.8)
    for short_label, x, y in zip(short, b, a):
        ax.annotate(short_label, (x, y), xytext=(5, 5), textcoords="offset points",
                     fontsize=8.5)
    ax.axhline(1, color="#6E5060", linewidth=0.7, linestyle="--")
    ax.axvline(0, color="#6E5060", linewidth=0.7)
...
def fig6_comfort_dominates(findings):
    ...
    colors = [PALETTE["primary"] if p < 0.001 else PALETTE["accent"] if p < 0.05 else PALETTE["muted"]
              for p in ps]
    ax.errorbar(cs, y, xerr=[err_lo, err_hi], fmt='o', color="#B5A0AB",
                  ecolor="#6E5060", capsize=3, markersize=8)
...
def fig8_robustness_matrix():
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            v = M_arr[i, j]
            if v == 1:
                ax.text(j, i, "✓", ha="center", va="center", fontweight="bold", color="#1A1024")
            elif v == 0.5:
                ax.text(j, i, "~", ha="center", va="center", fontweight="bold", color=PALETTE["late"])
            else:
                ax.text(j, i, "·", ha="center", va="center", color="#6E5060")
    ...
    ax.grid(which="minor", color="#3A2A36", linestyle="-", linewidth=0.5)
...
def fig10_cluster_profile(df):
    ...
    for i, (label, row) in enumerate(means.iterrows()):
        ...
        for m in metrics:
            ...
            normalized.append(v)
        ax.bar(x + (i - 1) * width, normalized, width, label=label, color=cs[i], edgecolor="#1A1024")
    ax.set_xticks(x)
